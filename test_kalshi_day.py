"""
Run: python3 test_kalshi_day.py
Shows Kalshi sports markets closing before 5 PM ET today.
"""
import os, sys
...
for m in all_markets:
    ...
    if not ct_str:
        # No close time — include by default in day bucket
        markets.append(m)
        continue
    try:
...
print(f"\n{'='*80}")
print(f"  KALSHI DAY MARKETS — {now.strftime('%b %d, %Y  %I:%M %p ET')}  ({len(markets)} markets before 5 PM ET)")
print(f"{'='*80}")
print(row_fmt(HDR))
print(sep)

if not markets:
    print("  No day markets found.")
else:
    for m in markets:
